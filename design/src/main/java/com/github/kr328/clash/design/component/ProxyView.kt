...
class ProxyView(
    ...
    override fun draw(canvas: Canvas) {
        ...
        canvas.apply {
            ...
            paint.color = state.config.accent
            paint.alpha = if (state.selected) 200 else 45
            drawPath(path, paint)
            ...
            if (state.selected) {
                ...
                drawRoundRect(
                    state.config.layoutPadding,
                    state.config.layoutPadding + 12f,
                    state.config.layoutPadding + 5f,
                    height - state.config.layoutPadding - 12f,
                    4f,
                    4f,
                    paint
                )
