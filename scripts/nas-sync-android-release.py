#!/usr/bin/env python3
"""Safely publish a Shenxianyun Android GitHub Release to NAS Dufs."""
...
REPOSITORY = "abxian/shenxianyun-android"
DEFAULT_DUFS_ROOT = Path("/vol1/dufs/data/sxy")
DEFAULT_WORK_ROOT = Path("/vol1/1000/docker-projects/shenxianyun-release-sync/work-android-sxnn")
DEFAULT_BACKUP_ROOT = Path("/vol1/1000/docker-projects/backups")
ALIASES = {"shenxianyun.apk": "arm64-v8a", "shenxianyunall.apk": "universal"}
STATE_FILE = ".android-sxnn-release-state.json"
...
def main() -> int:
    ...
    with (args.work_root / "sync.lock").open("w") as lock:
        ...
        with tempfile.TemporaryDirectory(prefix="android-sync-", dir=args.work_root) as temporary:
            ...
            if assets["shenxianyun.apk"]["digest"] != arm64 or assets["shenxianyunall.apk"]["digest"] != universal:
                raise SyncError("fixed APK aliases do not match arm64/universal assets")
            timestamp = datetime.now().strftime("%Y%m%d-%H%M%S-%f")
            backup = args.backup_root / f"sxy-android-sync-{args.tag}-{timestamp}"
